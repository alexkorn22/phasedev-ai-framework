{{phase_opening_summary}}Phase 5. Implementation.

Phase contract: complete only the current iteration of the approved implementation plan.
...
Current iteration from approved plan:
{{phase_excerpt}}

Ordered workflow:
1. Read this phase prompt, the embedded full-plan orientation, and the embedded current iteration excerpt first; open the full [iteration_plan.md]({{plan_path}}) only when patching current-iteration task checkboxes or `Check Evidence`, or when the embedded orientation/excerpt is missing or contradictory.
...
- do not block on PRD/design coverage gaps outside the current iteration boundary; mention them only as remaining risks if discovered while following the retrieval order;
- do not mark the iteration heading `[x]` at this phase. It must remain `[~]` until successful validation.

{{path_resolution_rule}}
- Only production/test/source/config changes from the current iteration belong outside `.phasedev/**`.
...
- `Risks: <remaining current-iteration risks or "none">`

## Artifact allowlist

Allowed persistent artifacts for this phase:
- production/test code needed for current iteration
