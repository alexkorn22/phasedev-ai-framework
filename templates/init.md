...
This prompt initializes context only. It is not a phase contract.

## Init Role

`phasedev init` is a context-only handshake before the state-driven `phasedev phase` loop. The source of truth for executable work is the complete controller output printed by `phasedev phase`, not chat history or this init prompt.
...
- Do not infer, reconstruct, summarize, or execute a phase contract from memory, the flow model, nearby user text, or chat history.
- `phasedev init` does not inject phase-specific skill policy. Skill policy appears only in executable `phasedev phase` phase prompts.
...
3. Stop until the user provides the complete, verbatim `phasedev phase` controller output.
...
```text
Init acknowledged. I will make no file changes, run no commands, and wait for the complete `phasedev phase` controller output.
```

Do not add a plan, checklist, file summary, assumptions, or next-phase instructions.

## Next Input Handling

- Valid next input is the complete, verbatim controller output printed by `phasedev phase`, including a phase heading and the full phase-specific executable contract.
...
- If the next input is missing, partial, paraphrased, conflicting, or descriptive-only, ask for the complete `phasedev phase` controller output and perform no work.
- Do not execute if the user explicitly says stop, pause, cancel, or not yet.
- Once a valid `phasedev phase` contract is accepted, this init prompt does not add extra execution rules; follow only the controller contract plus higher-priority/system/developer/user safety instructions.
...
- For incomplete next input, no work is performed and the agent asks for complete `phasedev phase` controller output.
