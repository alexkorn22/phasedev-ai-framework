...
2. **Scope / design / plan feedback** (requirements change, different architecture, re-planning):
   ...
   - Scope change on an already-final-validated change: if final validation already passed (`validation_findings.md` has `verdict: ready` or `ready_with_risks` with `type: final`) and your scope change adds a new iteration to `iteration_plan.md`, do NOTHING extra to `validation_findings.md`. The CLI auto-resets the stale terminal verdict to `pending` during `phasedev sync-state`/`phasedev advance` (findings `type` normalization keys on the locked validation phase, so `type: final` stays until `phasedev advance` enters the new iteration's validation, which flips it to `type: iteration`); hand-editing `validation_findings.md` remains forbidden.
