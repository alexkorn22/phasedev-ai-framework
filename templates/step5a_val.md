...
- current-phase requirements conformance pass: verify that the actual changed code implements exactly the current-phase requirements from PRD, approved design, and implementation plan; if behavior is missing, extra, contradictory, or only implied by `Check Evidence`, add a finding;
